## Data gathering: Updated 10th July 2019 ##
...
speaker.type.n <- sibsdata %>%
  ...
  ungroup() %>%
  replace(is.na(.), 0) %>%
  mutate(Total.input = (MOT + FAT + SIBLING + TV + TOY),
...
speaker.type.PC <- speaker.type.n %>% 
...
speaker.type <- speaker.type.n %>%
  dplyr::select(subj, month, MOT, FAT, SIBLING, TOY, TV) %>%
  gather(`MOT`,`FAT`, `SIBLING`, `TV`, `TOY`, 
         key = "Speaker", 
         value = "n") %>% 
