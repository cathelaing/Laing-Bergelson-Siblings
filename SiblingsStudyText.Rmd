...
With regard to the infants’ linguistic environment, we hypothesize that increased sibling number will have a negative effect on factors of the early input that are known to support language development. To test this, we adopt three input measures, established in the literature as being important for early language learning. The measures and our specific predictions are as follows:

1)	**Amount of input** will be lower for children with more siblings. Caregivers’ attention will be divided across a larger group of children, and as a result the proportion of input from the mother will be lower for infants with more siblings. In addition, these infants will experience more input from other children in the home. Amount of overall input is an important predictor of language development in the longer term [@huttenlocher_early_1991], as is amount of one-to-one input from the caregiver [@ramirez-esparza_look_2014], while overall speech in the environment (i.e. including speech not necessarily directed at the infant) does not appear to support lexical development (*ibid*). We thus expect amount of input directed towards the child to be an important determiner of infants’ lexical production skills at 18 months.
<!-- eb2cl: revisit wording -->
2)	**More 'learnable' words** [words that tend to be acquired earlier, @fenson_variability_1994] will occur in the input of infants with fewer siblings than the input of those with more siblings. This prediction stems from the idea that input from older children will generate a wider variety of lexical items in the input, including words directed to and produced by siblings, some of whom will still be young language learners themselves. As a result, in homes with older siblings, we predict that the lexical items in the input will be less typical of words acquired earlier on in development.
<!-- eb2cl: i'm not sure about the "learnable" term, guava isn't any less learnable than apple but only the latter is on the cdi. revisit. -->
3)	**Amount of object presence** (word + object co-occurrence, e.g. mother says 'cat' when there is a cat in the room) will decrease as sibling number increases. This predictions stems from the idea that as caregivers’ attention is drawn away from one-to-one interactions with the infant, there will be less opportunity for contingent talk. Moreover, less "learnable" words are also expected to be less imageable, and thus less likely to be presented alongside caregivers’ utterances. The co-occurrence of words alongside their referents is thought to contribute to the earlier learning of nouns over verbs [@bergelson_acquisition_2013], as it supports the word learning process through the concrete mapping of word to referent [@gleitman_hard_2005]. Furthermore, object presence is more suited to instances of joint visual attention with the caregiver, which is less likely to occur in households where caregiver attention is shared between multiple children.
<!-- eb2cl: revisit wording -->
...
**Parental report data** We draw on data from vocabulary checklists [Macarthur-Bates Communicative Development Inventory, hereafter CDI; @fenson_variability_1994], administered monthly from 0;6 to 1;6, along with a demographics questionnaire. Because the majority of infants did not produce their first word until around 0;11 according to CDI reports (M=`r firstword$meanfirstword`, SD=`r firstword$sdfirstword`)[^1], we use CDI data from 0;10 onwards in our analysis. CDI production data for each month is taken as a measure of the infants’ lexical development over the course of the analysis period. Supplementing this parental report, we also consider words produced in the hour-long video- and day-long audio-recordings.
...
[^5]: In the audio data, the model comparison revealed a significant effect of sibling group on shared book reading (*p*=.04), but post-hoc comparisons showed no significant difference between sibling groups (*ps*>.01).
