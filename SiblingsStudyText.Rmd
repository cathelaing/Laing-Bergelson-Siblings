...
We next modeled the effect of siblings on reported productive vocabulary. We tested three variables representing the sibling effect: a binary variable (0 vs. >0 siblings), aggregated groups (None vs. One vs. 2+), and discrete sibling number (0 vs. 1 vs. 2 vs. 3 vs. 4 siblings) using the model structure *vocabulary size (log-transformed) ~ siblings [binary, group or discrete] + age (months) + (1|subj)*. 
<!-- Outcomes from model comparisons are shown in Table \@ref(tab:table-sibling-model-output).  -->
In our sample, the simple fact of having siblings (i.e. as a binary variable) did not affect productive vocabulary, while both discrete sibling number and sibling group predicted reported CDI vocabulary size. 
...
```{r table-sibling-model-summary, echo=FALSE}
