...
table.sibling.model.output <- rbind(sibsYN_anova, sibgroup_anova, sibnumber_anova
  ...
  cbind(models_r2) %>%
  rename("R2" = "models_r2") # need to change this to formula
...
While we operationalized caregiver input in our models as input speech from the two adults who produced the most words in any given session, in `r round(PCMOTFAT,2)`% of cases this was the mother or father. Considering mothers and father specifically, maternal input accounted for `r round(PC.MOT$PCMOT)`% of object words in the data overall (M=`r mean.input.MOT$mean.input` words, Mdn=`r mean.input.MOT$med.input`, SD=`r mean.input.MOT$sd.input`)[^4]. Fathers accounted for an average of `r round(PC.FAT$PCFAT)`% (M=`r mean.input.FAT$mean.input`, Mdn=`r mean.input.FAT$med.input`, SD=`r mean.input.FAT$sd.input`), while infants with siblings received around `r round(PC.SIB$PCSIB)`% of their input from their brothers and sisters (M=`r mean.input.SIB$mean.input`, Mdn=`r mean.input.SIB$med.input`, SD=`r mean.input.SIB$sd.input`). See Table \@ref(tab:table-data-summary) and Figure \@ref(fig:Figure-Speaker-count). As well as the overall input being greater for firstborns, compared with infants with one or 2+ siblings, note also that the variance is greater for this group, and decreases  as sibling number increases. This is shown in the SDs reported in Table \@ref(tab:table-data-summary), and in the data points visualized in Figure \@ref(fig:Figure-Speaker-count).
...
```{r figure-sibpresence, echo=FALSE, fig.cap=cap, fig.pos='H', message=FALSE, warning=FALSE}
par(cex = 1.2)
multiplot(Figure.input.sibling, Figure.object.presence.sibling, cols=2)
cap <- sprintf("Difference in number of input words and %% of object presence in infants' input according to whether or not a sibling or siblings were present during the time of recording. Infants with no siblings were not included in the plots for visual ease. White shapes represent individual infants in the data; filled shapes represent means and 95%%CIs. For both plots, colours represent presence or absence of siblings during the recording session.")
...
```{r sibling presence - input, include=FALSE}

speaker.type.sib.mean <- sib.presence.input %>% 
  group_by(subj, sib.present) %>% 
  mutate(MeanN = mean(n)) %>% 
  filter(month == 17) 

sib.input_wilcox_one <- wilcox.test(
  x = subset(speaker.type.sib.mean, SibGroup == "One" & sib.present == "Sibling not present")$MeanN
  , y = subset(speaker.type.sib.mean, SibGroup == "One" & sib.present == "Sibling present")$MeanN
  , paired = FALSE,
  conf.int = TRUE
)

sib.input_wilcox_one_confint <- as.data.frame(sib.input_wilcox_one$conf.int)

sib.input_wilcox_two <- wilcox.test(
  x = subset(speaker.type.sib.mean, SibGroup == "2+" & sib.present == "Sibling not present")$MeanN
  , y = subset(speaker.type.sib.mean, SibGroup == "2+" & sib.present == "Sibling present")$MeanN
  , paired = FALSE,
  conf.int = TRUE
)

sib.input_wilcox_two_confint <- as.data.frame(sib.input_wilcox_two$conf.int)

sib.input_wilcox_one_two_present <- wilcox.test(
  x = subset(speaker.type.sib.mean, SibGroup == "2+" & sib.present == "Sibling present")$MeanN
  , y = subset(speaker.type.sib.mean, SibGroup == "One" & sib.present == "Sibling present")$MeanN
  , paired = FALSE,
  conf.int = TRUE
)

sib.input_wilcox_one_two_present_confint <- as.data.frame(sib.input_wilcox_one_two_present$conf.int)

sib.input_wilcox_one_two_not_present <- wilcox.test(
  x = subset(speaker.type.sib.mean, SibGroup == "2+" & sib.present == "Sibling not present")$MeanN
  , y = subset(speaker.type.sib.mean, SibGroup == "One" & sib.present == "Sibling not present")$MeanN
  , paired = FALSE,
  conf.int = TRUE
)

sib.input_wilcox_one_two_not_present_confint <- as.data.frame(sib.input_wilcox_one_two_not_present$conf.int)


```

```{r sibling presence - object presence, include=FALSE}

object.presence.sib.summary <- sib.presence.OP %>%
  group_by(subj, SibGroup, sib.present) %>%
  summarise(mean.object = mean(PC, na.rm=T),
            sd.object = sd(PC, na.rm=T))

sib.object.presence_wilcox_one <- wilcox.test(
  x = subset(object.presence.sib.summary, SibGroup == "One" & sib.present == "Sibling not present")$mean.object
  , y = subset(object.presence.sib.summary, SibGroup == "One" & sib.present == "Sibling present")$mean.object
  , paired = FALSE,
  conf.int = TRUE
)

sib.object.presence_wilcox_one_confint <- as.data.frame(sib.object.presence_wilcox_one$conf.int)

sib.object.presence_wilcox_two <- wilcox.test(
  x = subset(object.presence.sib.summary, SibGroup == "2+" & sib.present == "Sibling not present")$mean.object
  , y = subset(object.presence.sib.summary, SibGroup == "2+" & sib.present == "Sibling present")$mean.object
  , paired = FALSE,
  conf.int = TRUE
)

sib.object.presence_wilcox_two_confint <- as.data.frame(sib.object.presence_wilcox_two$conf.int)

sib.object.presence_wilcox_one_two_present <- wilcox.test(
  x = subset(object.presence.sib.summary, SibGroup == "2+" & sib.present == "Sibling present")$mean.object
  , y = subset(object.presence.sib.summary, SibGroup == "One" & sib.present == "Sibling present")$mean.object
  , paired = FALSE,
  conf.int = TRUE
)

sib.object.presence_wilcox_one_two_present_confint <- as.data.frame(sib.object.presence_wilcox_one_two_present$conf.int)

sib.object.presence_wilcox_one_two_not_present <- wilcox.test(
  x = subset(object.presence.sib.summary, SibGroup == "2+" & sib.present == "Sibling not present")$mean.object
  , y = subset(object.presence.sib.summary, SibGroup == "One" & sib.present == "Sibling not present")$mean.object
  , paired = FALSE,
  conf.int = TRUE
)

sib.object.presence_wilcox_one_two_not_present_confint <- as.data.frame(sib.object.presence_wilcox_one_two_not_present$conf.int)

```

We ran Wilcoxon tests to determine the extent of these differences in the data. Note that in some cases, individual infants were recorded both with and without their sibling(s) across different sessions, meaning that the data takes into account mean 'sibling present' and 'sibling not present' values for each individual infant. First, looking at amount of input words, there was no statistical difference in the amount of input words produced by the caregivers when siblings were present vs. when they weren't, for either sibling group (One: *W*=`r round(sib.input_wilcox_one$statistic)`, *p*=`r printp(sib.input_wilcox_one$p.value)`, CI=[`r (sib.input_wilcox_one_confint[1,])`,`r (sib.input_wilcox_one_confint[2,])`]; 2+: *W*=`r round(sib.input_wilcox_two$statistic)`, *p*=`r printp(sib.input_wilcox_two$p.value)`, CI=[`r (sib.input_wilcox_two_confint[1,])`,`r (sib.input_wilcox_two_confint[2,])`]). However, when siblings were present, infants with one sibling heard more input words than infants with two or more siblings (*W*=`r round(sib.input_wilcox_one_two_present$statistic)`, *p*=`r printp(sib.input_wilcox_one_two_present$p.value)`, CI=[`r (sib.input_wilcox_one_two_present_confint[1,])`,`r (sib.input_wilcox_one_two_present_confint[2,])`]) (though note that the exact number of siblings present is not taken into account here). When siblings were not present, there were no group differences (*W*=`r round(sib.input_wilcox_one_two_not_present$statistic)`, *p*=`r printp(sib.input_wilcox_one_two_not_present$p.value)`, CI=[`r (sib.input_wilcox_one_two_not_present_confint[1,])`,`r (sib.input_wilcox_one_two_not_present_confint[2,])`]). 

Next, comparing object presence within groups, we found that amount of object presence was significantly higher when a sibling was not present, across both groups (One: *W*=`r round(sib.object.presence_wilcox_one$statistic)`, *p*=`r printp(sib.object.presence_wilcox_one$p.value)`, CI=[`r (sib.object.presence_wilcox_one_confint[1,])`,`r (sib.object.presence_wilcox_one_confint[2,])`]; 2+: *W*=`r round(sib.object.presence_wilcox_two$statistic)`, *p*`r printp(sib.object.presence_wilcox_two$p.value)`, CI=[`r (sib.object.presence_wilcox_two_confint[1,])`,`r (sib.object.presence_wilcox_two_confint[2,])`]). Comparing between groups, object presence was higher for infants with one versus 2 or more siblings even when a sibling was present (*W*=`r round(sib.object.presence_wilcox_one_two_present$statistic)`, *p*=`r printp(sib.object.presence_wilcox_one_two_present$p.value)`, CI=[`r (sib.object.presence_wilcox_one_two_present_confint[1,])`,`r (sib.object.presence_wilcox_one_two_present_confint[2,])`]), but there was no difference between sibling groups when siblings were not present (*W*=`r round(sib.object.presence_wilcox_one_two_not_present$statistic)`, *p*=`r printp(sib.object.presence_wilcox_one_two_not_present$p.value)`, CI=[`r (sib.object.presence_wilcox_one_two_not_present_confint[1,])`,`r (sib.object.presence_wilcox_one_two_not_present_confint[2,])`]). Note that these effects should be interpreted with caution given the exploratory nature of this analysis. 
...
```{r correlations between CDI scores and other measures, include = FALSE}
