...
apa_table(table.sibling.model.output, caption = cap, col_spanners = NULL, digits = 2, placement = "H", 
          font_size = "small",
          note = "While we report R2 values for completeness, we note that utility and interpretability of this metric for this model type is debated, see https://bbolker.github.io/mixedmodels-misc/glmmFAQ.html.")
```
...
# eb2cl
# i made a related note in the gdoc, shouldn't this be split by sib group?
# Also why spearman not pearson? and sentence needs p-values or range of pvals (all p<.001) or wahtever
# also the sentence below underspecifies what you did which appears to be just from month 17, right? that's fine just say so (since that's when vocab is biggest), but just tob e sure: totalwords is from JUST month 17 cdi? and types/tokens is just month 17 too? or total they've ever said?
...
### Limitations
Of course, the ‘success’ of early language development is defined by how success is measured. Here we chose word production as our measure of linguistic capability; we did not consider other equally valid measures such as language comprehension or early social-interaction skills. Similarly, our input measures focused on nouns; other lexical classes may reveal different effects, though they are generally far sparser in production until toddlerhood. Our analysis of vocabulary relied on parental report data; this method could have biased our first-born sample towards more accurate or larger vocabulary reports owing to their parents having more time and attention to spend observing their vocabulary development [see @kartushina_covid-19_2022 for a discussion of this possibility in light of the COVID-19 pandemic, though note the present data were collected in 2014-16]. However, we were able to validate this measure by running correlation tests between reported (CDI) vocabulary and the number of word types produced by each infant in both the audio and video data. Looking at the data taken at 17 months (the last datapoint where we have both CDI and recorded measures), reported productive vocabulary size correlated strongly with number of word types produced in both the audio and video data for infants in the None/One sibling groups (video: *r*s > `r round(max(subset(measures.corrs, SibGroup != "2+")$cdi_vid_est,2))`, *p*s`r printp(min(subset(measures.corrs, SibGroup != "2+")$cdi_vid_pval))`; audio: *r*s > `r round(max(subset(measures.corrs, SibGroup != "2+")$cdi_aud_est,2))`, *p*s`r printp(min(subset(measures.corrs, SibGroup != "2+")$cdi_aud_pval))`). However, there was no correlation between these measures for infants in the 2+ group (*r*s > `r round(min(measures.corrs2$est,2))`, *p*s>`r printp(min(measures.corrs2$value))`)
