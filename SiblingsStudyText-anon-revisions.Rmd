...
siblinggroup1.beta <- (1-(exp(as.numeric(sibgroup.model.summary.table$Estimate[2]))))*100
siblinggroup2.beta <- (1-(exp(as.numeric(sibgroup.model.summary.table$Estimate[3]))))*100
...
cap <- sprintf("Mean number of words produced by Mothers, Fathers and Siblings, as well as mean total family input (mother + father + sibling(s)), across sessions recorded between 10-17 months. Circles represent values for individual infants; red triangles show group means. In the case where the infant had two mothers, mean maternal input is shown. See S7 for a month-by-month visualisation of caregiver input.")
```
...
objpres1.beta <- (1-(exp(as.numeric(object.presence.model.coefs$Estimate[2]))))*100
objpres2.beta <- (1-(exp(as.numeric(object.presence.model.coefs$Estimate[3]))))*100
