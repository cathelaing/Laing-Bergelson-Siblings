...
So far, our analysis takes into account the differences in input based on whether or not the target child has a sibling, but does not directly consider whether sibling presence affected these variables. If it is the presence of the sibling that affects the input, then we would expect to see a difference in our input measures when the sibling is present vs. absent. On the other hand, if the very fact of having a sibling changes the way that a caregiver interacts with the infant, then no difference should be observed. Sibling presence was not coded directly in the dataset, but we can use sibling speech as a proxy of their presence (i.e., if speech from a sibling was recorded, we know they were present; if no speech was recorded we assume that they were not present). As reported above, at least one sibling was present in `r round(sib.presence.summary$mean.present*100,0)`% of recordings for the infants who had a sibling. 
