...
We tested overall quantity of input (aggregated across the two main caregivers, as outlined above, and siblings) in our model alongside age, sex and subject, as noted above, and a significant effect was found for the effect of sibling group ($\chi^2 (`r speaker.type_df`)$ = `r speaker.type_chisq`, *p*=`r printp(speaker.type_p.value)`, $R^2$=`r round(speaker.type_r2,2)`). Infants with one sibling heard on average `r round((((subset(table.data.summary.speaker, SibGroup == "None")$mean)-(subset(table.data.summary.speaker, SibGroup == "One")$mean))/(subset(table.data.summary.speaker, SibGroup == "One")$mean))*100,0)`% fewer words than those with no siblings in any given hour-long recording, while infants with two or more siblings heard `r round((((subset(table.data.summary.speaker, SibGroup == "None")$mean)-(subset(table.data.summary.speaker, SibGroup == "2+")$mean))/(subset(table.data.summary.speaker, SibGroup == "2+")$mean))*100,0)`% fewer words.
