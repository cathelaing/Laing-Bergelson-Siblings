...
authornote: |
  This work was funded by NIH Grant DP5-OD019812 to EB. Special thanks to Bergelson Lab trainees, staff, and RAs at Duke and Rochester (and especially Shannon Dailey, Andrei Amatuni and Hallie Garrison). The authors declare no conflicts of interest with regard to the funding source for this study. Supplementary materials, including code, for this manuscript can be found at  https://osf.io/rm3ec/. 
...
```{r caregiver 1 and 2, include=FALSE}
...
Our analyses consider infants’ total productive vocabulary[^2] alongside our two input measures -- nouns in household input and extent of object presence in the input -- as a function of sibling number.  
...
speaker.type_video %>% filter(caregiver == "FAMILY") %>%
  group_by(month) %>% 
...
age.test.input <- lmer(Log.n ~ month + (1|subj), data = subset(speaker.type_video, 
                                                               caregiver == "FAMILY"),
                       REML=FALSE)
age.test.input.null <- lmer(Log.n ~ (1|subj), data = subset(speaker.type_video, 
                                                            caregiver == "FAMILY"),
                            REML=FALSE)
...
sex.test.input <- lmer(Log.n ~ sex + (1|subj), data = subset(speaker.type_video, 
                                                             caregiver == "FAMILY"),
                       REML=FALSE)

sex.test.input.null <- lmer(Log.n ~ (1|subj), data = subset(speaker.type_video, 
                                                            caregiver == "FAMILY"),
                            REML=FALSE)
...
age.test.OP <- lmer(PC ~ month + (1|subj), data = object.presence_video, REML=FALSE)
age.test.OP.null <- lmer(PC ~ (1|subj), data = object.presence_video, REML=FALSE)
...
sex.test.OP <- lmer(Log.n ~ sex + (1|subj), data = object.presence_video, REML=FALSE)
sex.test.OP.null <- lmer(Log.n ~ (1|subj), data = object.presence_video, REML=FALSE)
...
#shapiro.test(speaker.type_video$n) # not normal

speaker.type.model.null <- lmerTest::lmer(Log.n ~ month + sex + (1|subj), 
                                          data=subset(speaker.type_video, 
                                                      caregiver == "FAMILY"),
                                          REML=FALSE)

speaker.type.model <- lmerTest::lmer(Log.n ~ SibGroup + month + sex + (1|subj), 
                                     data=subset(speaker.type_video, 
                                                 caregiver == "FAMILY"),
                                     REML=FALSE)
...
speaker.type.mean <- speaker.type_video %>% 
...
speaker.type_wilcox_none_one <- wilcox.test(
  x = subset(speaker.type.mean, SibGroup == "None")$MeanN
  , y = subset(speaker.type.mean, SibGroup == "One")$MeanN
  , paired = FALSE,
  conf.int= TRUE
)
...
speaker.type_wilcox_one_two <- wilcox.test(
  x = subset(speaker.type.mean, SibGroup == "2+")$MeanN
  , y = subset(speaker.type.mean, SibGroup == "One")$MeanN
  , paired = FALSE,
  conf.int= TRUE
)
...
nMOTFAT <- speaker.type_video %>% filter(caregiver %in% c("CG1", "CG2") & 
                                     speaker %in% c("MOT", "MT2", "FAT")) %>% nrow()

nALL <- speaker.type_video %>% filter(caregiver %in% c("CG1", "CG2")) %>% nrow()

PCMOTFAT <- (nMOTFAT/nALL)*100

non_parents <- nALL <- speaker.type_video %>% filter(!is.na(speaker) & 
                                                 !(speaker %in% c("SIBLING", "MOT", "MT2", "FAT"))) 

all.speakers <- speaker.type_video %>%
  filter(caregiver != "FAMILY") %>%
  group_by(subj) %>%
...
mean.input.MOT <- speaker.type_video %>% group_by(subj) %>%
  filter(speaker == "MOT" | speaker == "MT2") %>% 
...
mean.input.FAT <- speaker.type_video %>% ungroup() %>%
  filter(speaker == "FAT") %>% summarise(mean.input = mean(n),  # 18.14
                                                                          med.input = median(n),
                                                                          sd.input = sd(n))      # 42.01

mean.input.SIB <- speaker.type_video %>% ungroup() %>%
  filter(speaker == "SIBLING" & SibGroup != "None") %>% 
...
object.presence.summary <- object.presence_video %>%
...
#shapiro.test(object.presence_video$PC) # not normal

object.presence.model <- lmerTest::lmer(PC ~ SibGroup + month + (1|subj), data = object.presence_video,
                                        REML=FALSE)

object.presence.model.null <- lmerTest::lmer(PC ~ month + (1|subj), data = object.presence_video, 
                                             REML=FALSE)
