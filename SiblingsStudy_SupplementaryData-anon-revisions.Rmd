...
We re-plotted Figures 2 and 3 from the main paper with age on the x-axis, to determine whether there was any age-related changes in these variables. Note that age was a significant factor in both linear mixed effects models testing caregiver input/object presence as dependent variables.
...
cap <- sprintf("Amount (%%) of object presence in infants' input over time (n=43). Colors denote sibling group; line with colored confidence band reflects local estimator (loess) fit over object presence at each month. Triangles indicate mean with bootstrapped CIs computed over each month's data. Points (jittered horizontally) show object presence in individual infants' inputs at each month.")
